...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val list: ArrayList<dataClass> = arrayListOf()
        list.add(
            dataClass(
        ...
        recyclerview.run {
            ...
            adapter = LeagueAdapter(this@MainActivity,list) {
                click(it)
            }
        }
    }

    private fun click(dataClass: dataClass) {
        Log.d(
            "CLOG",
            "nama = " + dataClass.title + " desc = " + dataClass.description + " gambar = " + dataClass.imageView
        )

        startActivity<Detail>(
            "title" to dataClass.title,
            "desc" to dataClass.description,
            "image" to dataClass.imageView
        )
    }
}
